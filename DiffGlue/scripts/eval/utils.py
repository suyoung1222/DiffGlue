...
def eval_relative_pose_dlt(data, pred, conf):
    check_keys_recursive(data, ["view0", "view1", "T_0to1"])
    check_keys_recursive(
        pred, ["keypoints0", "keypoints1", "matches0", "matching_scores0"]
    )
    ...
    if not est["success"]:
        results["rel_pose_error_dlt"] = float("inf")
        results["ransac_inl_dlt"] = 0
        results["ransac_inl%_dlt"] = 0
    else:
        # R, t, inl = ret
        M = est["M_0to1"]
        inl = est["inliers"].numpy()
        t_error, r_error = relative_pose_error(T_gt, M.R, M.t)
        results["rel_pose_error_dlt"] = max(r_error, t_error)
        results["ransac_inl_dlt"] = np.sum(inl)
        results["ransac_inl%_dlt"] = np.mean(inl)
