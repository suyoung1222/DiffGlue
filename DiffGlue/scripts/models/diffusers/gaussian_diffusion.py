...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        model_output_all = model(x, self._scale_timesteps(t), **model_kwargs)
        model_output = model_output_all["adj_mat"]
        model_output_all.pop("adj_mat")
        ...
        def process_xstart(x):
            if denoised_fn is not None:
                x = denoised_fn(x)
            if clip_denoised:
                return x.clamp(-0.5*self.scale, 0.5*self.scale)
            return x
        ...
        result = {**model_output_all, **result}
        return result
    ...
    def p_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            adj_mat = th.randn(*shape, device=device)
            adj_mat = adj_mat.clamp(-0.5*self.scale, 0.5*self.scale)
            adj_mat[...,-1,-1] = 0
        # indices = list(range(self.num_timesteps))[::-1] # original DDIM
        indices = list(range(1))[::-1] # 1 step for DiffGlue's inference
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            terms["diffusion_loss"] = self._vb_terms_bpd(
                model=model,
                x_start=x_start,
                x_t=x_t,
                t=t,
            ...
            if self.loss_type == LossType.RESCALED_KL:
                terms["diffusion_loss"] *= self.num_timesteps
            ...
            if "vb" in terms:
                terms["diffuser_loss"] = terms["mse"] + terms["vb"]
            else:
                terms["diffuser_loss"] = terms["mse"]
        else:
            raise NotImplementedError(self.loss_type)

        terms = {**model_output_all, **terms}
        return terms
