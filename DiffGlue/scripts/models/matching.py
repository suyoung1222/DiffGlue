...
class Matching(torch.nn.Module):
    ...
    def __init__(self, config={}):
        ...
        ckpt_path = Path(ckpt) if not isinstance(ckpt, Path) else ckpt
        ...
        if ckpt_path.stat().st_size == 0:
            raise ValueError(
                f"Checkpoint file is empty: {ckpt_path}\n"
                f"The file exists but has no content. It may be corrupted or incomplete."
            )
        try:
            ...
            raise RuntimeError(
                ...
                f"Error: {str(e)}\n"
                f"The checkpoint file may be corrupted. Please verify the file is a valid PyTorch checkpoint."
            ) from e
