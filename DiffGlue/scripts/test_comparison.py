...
from demo.models.matching import Matching as DiffGlueMatching
from demo.models.superpoint import SuperPoint

# Try to import LoFTR - use try/except with different import methods
LOFTR_AVAILABLE = False
try:
    # Try relative import first (when run as module)
    from scripts.models.matchers.LoFTR.src.loftr import LoFTR
    from scripts.models.matchers.LoFTR.src.config.default import get_cfg_defaults
    LOFTR_AVAILABLE = True
except (ImportError, ValueError):
    try:
        # Try absolute import
        script_dir = Path(__file__).parent.parent
        loftr_path = script_dir / "scripts" / "models" / "matchers" / "LoFTR" / "src" / "loftr.py"
        config_path = script_dir / "scripts" / "models" / "matchers" / "LoFTR" / "src" / "config" / "default.py"
